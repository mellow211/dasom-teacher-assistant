...
export async function POST(request: Request) {
  ...
  try {
    const plan = await generateStructuredAiData(buildLessonPlanPrompt(input), lessonPlanJsonSchema, value => validateLessonPlanOutput(value,input), request.signal);
    return Response.json({ plan, overview: { grade:input.grade, subject:input.subject, topic:input.topic, session:`${input.currentSession}/${input.totalSessions}차시`, achievementStandard:input.achievementStandard } });
  } catch (error) {
